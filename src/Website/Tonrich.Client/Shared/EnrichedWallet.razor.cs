...
public partial class EnrichedWallet
{
    ...
    protected override async Task OnAfterFirstRenderAsync()
    {
        ...
        await Task.WhenAll(
                 ...
                 Task.Run(async () =>
                 {
                     var telegramNumbers = await TonService.GetNFTsAsync(AccountInfo.Raw, FixedConfig.TelegramAnonymousCollectionAddress);
                     ...
                     var telegramUserNames = await TonService.GetNFTsAsync(AccountInfo.Raw, FixedConfig.TelegramUserNameCollectionAddress);
    ...
    private static List<IGrouping<DayOfWeek, (int WeekInMonth, DateTimeOffset DateTime)>> GenerateActivityChartDates()
    {
        ...
        var minDate = maxDate.AddDays(-FixedConfig.TransactionsTimePeriodPerDays + 1);

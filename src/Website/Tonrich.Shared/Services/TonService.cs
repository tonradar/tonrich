...
public partial class TonService : ITonService
{
    ...
    public async Task<TransactionInfoDto?> GetTransactionsAsync(string raw, CancellationToken cancellationToken = default)
    {
        ...
        var from = dtNow.AddDays(-FixedConfig.TransactionsTimePeriodPerDays);
        var pageCount = 100;
        ...
        var deposit = transactions.Sum(c => c.DepositTon(raw)) / FixedConfig.TONDenominator;
        var spent = transactions.Sum(c => c.SpentTon(raw)) / FixedConfig.TONDenominator;


        var result = new TransactionInfoDto
        {
            DepositLastMonth = lastMonthTransactions.Sum(c => c.DepositTon(raw)) / FixedConfig.TONDenominator,
            SpentLastMonth = lastMonthTransactions.Sum(c => c.SpentTon(raw)) / FixedConfig.TONDenominator,
        ...
        var activities = transactions
            ...
            {
                ActivityDate = c.Key,
                ActivityAmount = c.Sum(c => Math.Abs(c.Price(raw))) / FixedConfig.TONDenominator
            });
